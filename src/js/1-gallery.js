...
},
{
 ...
 description: 'Lighthouse Coast Sea',
  ...
  lightbox.on('show.simplelightbox', (event) => {
    ...
    event.simpleLightbox.content.appendChild(caption);
    setTimeout(() => {
      caption.classList.add('sl-visible');
    }, 250);
  });

  lightbox.on('close.simplelightbox', (event) => {
    const caption = event.target.querySelector('.sl-caption');
    if (caption) {
      caption.remove();
    }
  });  
